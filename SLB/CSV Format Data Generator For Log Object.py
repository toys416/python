...
def DepthLog_CSV(Depth):
    ...
    Data_Matrix = np.empty([Depth*2+1,151])                                                                                       #Variable
    Data = pd.DataFrame(Data_Matrix)
    ...
    #Add value every 1ft(100 Channels)
    row2 = 0
    for i in range(len(Data.index)):
        Data.iloc[row2,51:151] =random.randint(1,10)                                                                              #Variable
    ...
    Data_Frame = Data_Frame.mask(Data_Frame.eq(0), '')
    #Export CSV
    file = Data_Frame.to_csv('DepthLog From '+ '2000 ' + 'To '+ str(Depth+2000)+' ft.csv',
           encoding='utf-8', index=False,header=False,sep=',')                                                                    #Variable
    return file
